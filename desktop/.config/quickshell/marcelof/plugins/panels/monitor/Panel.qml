...
Panel {
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["monitor", "set-brightness", root.focusedMonitor, String(percent)]
  ...
  function toggleDisplay(name, enabled) {
    ...
    actionProc.command = ["monitor", "set-enabled", name, enabled ? "off" : "on"]
    if (!actionProc.running) actionProc.running = true
  }

  function setScale(scale) {
    actionProc.command = ["monitor", "set-scale", root.focusedMonitor, String(scale)]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  Process {
    ...
    // The monitor helper can race the hardware/driver and
