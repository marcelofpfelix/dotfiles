...
Panel {
  ...
  readonly property var mediaService: null  // Direct MPRIS labels remain available without Omarchy's media service.
  ...
  // This setup has no speaker-tuning virtual sink; the default PipeWire sink owns volume.
  ...
  function showVolumeOsd(volume) {
    if (bar && bar.barRoot) bar.barRoot.showVolumeOsd()
  }
  ...
  function setDefaultSink(node) {
    ...
    if (node.id !== undefined) Quickshell.execDetached(["wpctl", "set-default", String(node.id)])
  }
  ...
  Process {
    ...
    command: ["audio-sink-availability"]
