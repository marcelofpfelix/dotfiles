...
Panel {
  ...
  // Actions use Quickshell's native BlueZ API; this map keeps the panel responsive while BlueZ catches up.
  ...
  // 'x' forgets remembered devices through Quickshell's native BlueZ API.
  ...
  // rfkill persists the radio state across reboot; BlueZ follows the block.
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["rfkill", adapter.enabled ? "block" : "unblock", "bluetooth"])
  }
