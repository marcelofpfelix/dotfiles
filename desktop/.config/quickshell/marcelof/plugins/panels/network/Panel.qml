...
Panel {
  ...
  // `network-status verbose` samples (~1.5s apart via detailsPoll).
  ...
  readonly property bool canSelectBand: false  // No band-pinning backend on this host.
  ...
  readonly property bool canRunSpeedTest: false  // Keep the panel dependency-free.
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      root.bar.run("hypr-term network-dns Custom")
      root.close()
      return
    }
  ...
  Process {
    id: detailsProc
    command: ["network-status", "verbose"]
