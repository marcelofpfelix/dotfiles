...
Item {
  ...
  property string helperPath: Quickshell.env("HOME") + "/bin"
  ...
  function applySelected(row) {
    ...
    if (row.entryType === "image") {
      Quickshell.execDetached([root.helperPath + "/omarchy-clipboard-paste-file", "--copy-only", row.mime, row.path])
    } else if (row.fullText) {
      Quickshell.execDetached([root.helperPath + "/omarchy-clipboard-paste-text", "--copy-only", "--history-index", String(row.historyIndex)])
    }
  }
  ...
  function openSelected(row) {
    if (!row) return
    root.opened = false
    Quickshell.execDetached([root.helperPath + "/omarchy-clipboard-open", "--history-index", String(row.historyIndex)])
  }
  ...
  Process {
    id: primarySyncProc
    command: ["setpriv", "--pdeathsig", "TERM", "wl-paste", "--type", "text", "--watch", "wl-copy", "--primary", "--type", "text/plain"]
    onExited: watchRestartTimer.restart()
  }
  ...
  Timer {
    ...
    onTriggered: {
      ...
      if (!primarySyncProc.running) primarySyncProc.running = true
    }
  }
