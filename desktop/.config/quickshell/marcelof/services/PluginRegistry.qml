...
QtObject {
  ...
  function isEnabled(id) {
    ...
    if (!shellConfigProvider)
      return !!manifest && (manifest.__isFirstParty || enabledPluginIds.indexOf(key) !== -1)
    if (manifest) {
  ...
  function supportsKind(id, kind) {
    ...
    if (!manifest || !Array.isArray(manifest.kinds) || manifest.kinds.indexOf(kind) === -1)
      return false
    var entryKind = kind === barWidgetKind ? "barWidget" : kind
