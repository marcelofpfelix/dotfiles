...
- [x] `hypr-session status` and `hypr-session smoke` check required Wayland tools, Quickshell, notification owner, portals, clock sync, audio state, inactive dunst, legacy launcher masks, and optional `hyprpicker`.
