...
- [x] `qs-menu-smoke` opens, captures, closes, and checks Quickshell launcher/control/media/notification/calendar/wallpaper/screen/keybinding/clipboard/websearch/network/tray/power menu screenshots plus `contact.png` for visual regression checks; `desktop-doctor` now fails when the latest smoke run predates live Quickshell QML.
