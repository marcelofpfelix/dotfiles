...
- [x] `desktop-doctor` wraps the live Wayland smoke check with current profile, Quickshell status, picker helpers, audio state, screen-record status, and static browser wrapper flag checks.
...
- [x] `qs-menu-smoke` opens, captures, and closes Quickshell launcher/control/media/notification/calendar/wallpaper/screen/keybinding/clipboard/websearch/network/tray/power menu screenshots for visual regression checks.
