...
- [x] `desktop-doctor` wraps the live Wayland smoke check with current profile, Quickshell status, picker helpers, audio state, audioctl pause/resume regression checks, screenshot helper status, screen-record status, and static browser wrapper flag checks.
