...
- `Win+D`, `Win+Space`, `Win+/`, `Win+Ctrl+A`, `Win+Shift+E`, `Win+W`, `Win+S`, `Win+M`, brightness keys, volume keys, screenshots, web search, and power actions are mapped through Hyprland or local Quickshell IPC; `Win+Ctrl+V` and `Win+Ctrl+W` are explicitly routed to Controls so they do not fall through to `Win+V` split or `Win+W` close.
